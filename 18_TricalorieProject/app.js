...
// Storage Controller 
// we cld use session storage instead of local storage, but that will be gone once we end the session 
        ...
        updateItemStorage: function(updatedItem){
            ...
            items.forEach(function(item, index){
                if(updatedItem.id === item.id){
                    items.splice(index,1,updatedItem); // params: index whr item to be deleted is at,  how many to remove, and any new elemenet to insert in
                }
            });
            // re set ls
            localStorage.setItem('items',JSON.stringify(items));
